...
curl "https://slack.com/api/users.profile.get?token=${SLACK_API_TOKEN}" -s | jq --raw-output '.profile | with_entries(select(.key == "status_text" or .key == "status_emoji")) | @uri' > "$TMPDIR/slack-profile.txt"
